...
export class AuthController {
  ...
  async login(@GetUser() user: UserEntity): Promise<TLoginResponse> {
    return await this.authService.login(user);
  }

  @Post('/dashboard-login')
  @UseGuards(LocalAuthGuard)
  async dashboardLogin(@GetUser() user: UserEntity): Promise<TLoginResponse> {
  ...
  @Patch('/disable-2fa')
  @UseGuards(AccessTokenGuard)
  async disableTwoFactor(@GetUser() user: UserEntity) {
  ...
  validateOtp(@Req() req: Request): TLoginResponse {
  ...
  dashboardValidateOtp(@Req() req: Request): TLoginResponse {
    return req.user as TLoginResponse;
  }
  ...
  forgotPassword(@Req() req: Request): TForgotPasswordResponse {
  ...
  async resetPasswordForm(
    ...
    const hashedOneTimeToken = await this.authService.hashFingerprint(
      user.one_time_token,
    );
  ...
  @UseGuards(AccessTokenGuard)
  permissionProvider(@GetUser() user: UserEntity): UserEntity {
    return user;
  }

  @Get('/something')
  something(@Req() req: Request) {
