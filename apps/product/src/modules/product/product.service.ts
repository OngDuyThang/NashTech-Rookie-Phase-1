...
export class ProductService {
  ...
  async findList(
    ...
    const subcategoryIds =
      await this.categoryRepository.getAllSubcategoryIds(categoryIds);
    ...
    if (!isEmpty(subcategoryIds)) {
      query.andWhere('product.category_id IN (:...subcategoryIds)', {
        subcategoryIds,
      });
    }
    ...
    try {
      switch (sort) {
        ...
        case PRODUCT_SORT.PRICE_ASC:
          return await this.productsByPrice(
            query,
            page,
            limit,
            QUERY_ORDER.ASC,
  ...
  async remove(id: string): Promise<void> {
    ...
    try {
      ...
      const _ok = this.cartService
        .send({ cmd: SERVICE_MESSAGE.REMOVE_CART_ITEM }, id)
        .pipe(
          timeout(10000),
          catchError((e) => {
  ...
  private async initProductCache(): Promise<TCacheLastProduct> {
    const firstProductRecord = await this.productRepository
      .createQueryBuilder()
  ...
  private async setLastProductCache(cache: TCacheLastProduct): Promise<void> {
  ...
  private async getLastProductCache(): Promise<TCacheLastProduct> {
    try {
      const lastProduct = await this.cacheManager.get<TCacheLastProduct>(
        LAST_PRODUCT_CACHE_KEY,
      );
      if (!lastProduct) {
  ...
  async findPopularProducts(): Promise<ProductEntity[]> {
    const _popularProductIds = this.orderService
      .send({ cmd: SERVICE_MESSAGE.GET_POPULAR_PRODUCTS }, {})
      .pipe(
        timeout(10000),
        catchError((e) => {
          if (e instanceof TimeoutError) {
    ...
    const popularProductIds = (await lastValueFrom(
      _popularProductIds,
    )) as string[];
  ...
  async findProductOnCart(id: string): Promise<ProductSchema> {
    try {
