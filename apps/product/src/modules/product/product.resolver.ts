...
export class ProductResolver {
  ...
  @Query(() => [ProductEntity])
  async popularProducts(): Promise<ProductEntity[]> {
  ...
  @Query(() => ProductEntity)
  async product(@Args('id', UUIDPipe) id: string): Promise<ProductEntity> {
  ...
  async reviews(
    ...
    const [reviews, total] = await this.productService.findReviewsByProduct(
      product,
      queryDto,
    );
  ...
  @Query(() => [ProductEntity])
  async searchProducts(@Args('query') query: string): Promise<ProductEntity[]> {
