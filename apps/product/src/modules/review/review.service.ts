...
export class ReviewService {
  ...
  async findList(queryDto: ReviewQueryDto): Promise<[ReviewEntity[], number]> {
    const { page, limit, sort } = queryDto;
    const order =
      sort == REVIEW_SORT.DATE_ASC ? QUERY_ORDER.ASC : QUERY_ORDER.DESC;
  ...
  async getProductRatings(productId: string): Promise<number[]> {
    try {
